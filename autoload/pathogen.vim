" pathogen.vim - path option manipulation and simple plugin management tool
" Original Author: Tim Pope <http://tpo.pe/>
" Maintainers:     Israel Chauca <israelchauca at gmail dot com>
"                  Barry Arthur <barry dot arthur at gmail dot com>
" Version:         3.0
...
function! pathogen#infect(...) abort " {{{2
...
function! pathogen#split(path) abort " {{{2
...
function! pathogen#join(...) abort " {{{2
...
function! pathogen#legacyjoin(...) abort " {{{2
  return call('pathogen#join',[1] + a:000)
endfunction " }}}2

" Remove duplicates from a list.
function! pathogen#uniq(list) abort " {{{2
  let list = a:list
...
function! pathogen#separator() abort " {{{2
  return !exists("+shellslash") || &shellslash ? '/' : '\'
endfunction " }}}2

" Convenience wrapper around glob() which returns a list.
function! pathogen#glob(pattern) abort " {{{2
  let files = split(glob(a:pattern),"\n")
  return map(files,'substitute(v:val,"[".pathogen#separator()."/]$","","")')
endfunction "}}}2

" Like pathogen#glob(), only limit the results to directories.
function! pathogen#glob_directories(pattern) abort " {{{2
  return filter(pathogen#glob(a:pattern),'isdirectory(v:val)')
endfunction " }}}2

" Prepend all subdirectories of path to the rtp, and append all after
" directories in those subdirectories.
function! pathogen#runtime_prepend_subdirectories(path) " {{{2
...
" be used.
function! pathogen#runtime_append_all_bundles(...) " {{{2
  let sep = pathogen#separator()
...
function! pathogen#runtime_findfile(file,count) "{{{2
  let rtp = pathogen#join(1,pathogen#split(&rtp))
  return fnamemodify(findfile(a:file,rtp,a:count),':p')
endfunction " }}}2

" Backport of fnameescape().
function! pathogen#fnameescape(string) " {{{2
...
function! s:find(count,cmd,file,lcd) " {{{2
...
function! s:Findcomplete(A,L,P) " {{{2
  let sep = pathogen#separator()
  let cheats = {
        \'a': 'autoload',
        \'d': 'doc',
...
"}}}1
" Plugin Management Extensions {{{1

" parse all bundled_plugin files in &rtp
" NOTE: This (re)sets s:pathogen_disabled
function! pathogen#parse_bundled_plugins_files() " {{{2
...
function! pathogen#save_bundled_plugin_file() " {{{2
  let plugins = []
...
function! pathogen#cycle_filetype() " {{{2
...
function! pathogen#is_disabled(path) " {{{2
  if a:path =~# '\~$'
    return 1
...
function! pathogen#enable_plugin(plugin) " {{{2
...
function! pathogen#disable_plugin(plugin) " {{{2
  let plugin = tolower(a:plugin)
...
function! pathogen#list_plugins(arg) " {{{2
  let sep = pathogen#separator()
  let list = []
...
function! pathogen#is_disabled_plugin(path) " {{{2
...
function! s:call_vam(action, list) "{{{2
  try
    if a:action == 'install'
...
" Install files and its dependencies {{{2
...
function! s:command_complete(ArgLead, CmdLine, CursorPos) " {{{2
